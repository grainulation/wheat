...
describe('wheat guard', () => {
  ...
  it('allows writes to non-output paths', () => {
    const result = guard(tmpDir, JSON.stringify({
  ...
  it('blocks writes to output/ when no compilation.json exists', () => {
    ...
    assert.ok(result.reason.includes('BLOCKED'), 'reason should include BLOCKED');
  });
  ...
  it('allows writes to output/ when compilation is fresh and ready', () => {
    ...
    const now = new Date();
    fs.writeFileSync(compilationPath, JSON.stringify({
      status: 'ready',
      errors: [],
      summary: {},
    }));
    ...
    assert.equal(result.allow, true, 'should allow when compilation is fresh and ready');
  });
  ...
  it('blocks writes to output/ when compilation status is blocked', () => {
    ...
    assert.equal(result.allow, false);
    assert.ok(result.reason.includes('blocked'), 'reason should mention blocked status');
  });
  ...
  it('blocks claims.json writes missing meta.question', () => {
    ...
    assert.equal(result.allow, false);
    assert.ok(result.reason.includes('meta.question'), 'reason should mention meta.question');
  });
  ...
  it('blocks claims.json writes missing claims array', () => {
    ...
    assert.equal(result.allow, false);
    assert.ok(result.reason.includes('claims'), 'reason should mention claims array');
  });
