...
function toolAddClaim(dir, args) {
  ...
  if (!data) {
    return { status: 'error', message: loadErrors[0]?.message || 'Failed to load claims.json' };
  }
...
function toolSearch(dir, args) {
  const { data, errors: loadErrors } = loadClaims(dir);
  if (!data) {
    return { status: 'error', message: loadErrors[0]?.message || 'No claims.json found.' };
  }
...
function toolStatus(dir) {
  const { data, errors: loadErrors } = loadClaims(dir);
  if (!data) {
    return { status: 'no_sprint', message: loadErrors[0]?.message || 'No claims.json found. Run wheat init to start a sprint.' };
  }
...
function toolDeepwiki(_dir, args) {
  ...
  if (!repo || !/^[a-zA-Z0-9._-]+\/[a-zA-Z0-9._-]+$/.test(repo)) {
    return { status: 'error', message: 'Required: repo in "org/name" format (e.g., "grainulation/wheat")' };
  }

  const url = `https://deepwiki.com/${repo}`;

  return new Promise(resolve => {
    const req = https.get(url, { timeout: 15000, headers: { 'Accept': 'text/html' } }, res => {
      if (res.statusCode === 301 || res.statusCode === 302) {
        resolve({ status: 'ok', repo, url, note: `DeepWiki redirects for ${repo} — repo may not be indexed yet. Visit ${url} to trigger indexing.`, content: null });
        res.resume();
        return;
      }
      if (res.statusCode !== 200) {
        resolve({ status: 'error', message: `DeepWiki returned HTTP ${res.statusCode} for ${repo}. The repo may not be indexed — visit ${url} to trigger indexing.` });
        res.resume();
        return;
      }
    ...
    req.on('error', err => {
      resolve({ status: 'error', message: `Failed to reach DeepWiki: ${err.message}. Visit ${url} manually.` });
    });
    req.on('timeout', () => {
      req.destroy();
      resolve({ status: 'error', message: `DeepWiki request timed out. Visit ${url} manually.` });
    });
  });
}
...
function toolSyncLog(dir) {
  ...
  if (!fs.existsSync(logPath)) {
    return { status: 'ok', message: 'No sync history found. Use /sync to publish sprint artifacts.', entries: [] };
  }
  try {
...
const TOOLS = [
  ...
  {
    name: 'wheat/deepwiki',
    description: 'Fetch AI-generated documentation from DeepWiki for a public GitHub repo. Returns architecture overview, component descriptions, and section structure. Use with /pull to extract claims.',
    inputSchema: {
      type: 'object',
      properties: {
        repo: { type: 'string', description: 'GitHub repo in "org/name" format (e.g., "grainulation/wheat")' },
      },
      required: ['repo'],
    },
  },
  {
    name: 'wheat/sync-log',
    description: 'View the sync history — records of when sprint artifacts were published to Confluence, Slack, or other targets.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
...
const RESOURCES = [
  ...
  },
  {
    uri: 'wheat://sync-log',
    name: 'Sync History',
    description: 'Log of sprint artifact publishes to Confluence, Slack, Notion, etc.',
    mimeType: 'application/json',
  },
];
