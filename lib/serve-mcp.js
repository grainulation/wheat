...
import { jsonRpcError, jsonRpcResponse } from "@grainulation/barn/mcp";
import { isInsideDir } from "@grainulation/barn/paths";
...
async function handleRequest(dir, method, params, id) {
	switch (method) {
		...
		case "tools/call": {
			...
			if (!isInsideDir(effectiveDir, dir)) {
				return jsonRpcResponse(id, {
					content: [
						{
							type: "text",
		...
		case "resources/read": {
			...
			try {
				...
				return jsonRpcError(
					id,
					-32603,
					`Failed to read resource ${uri}: ${err.message}`,
				);
			}
			return jsonRpcResponse(id, {
...
function startServer(dir) {
	...
	// errors) bypass the per-request try/catch and would otherwise kill the
	// process silently, leaving Claude Code with a mysterious disconnect.
	// Log structured JSON to stderr (stdout is reserved for JSON-RPC framing)
	// so the parent plugin host can surface the crash, then exit with code 1
	// so the user sees a reload prompt rather than a hung transport.
	//
	// Registered inside startServer (not at module top) because this file is
	// also exported as a library (@grainulation/wheat/mcp); library consumers
	// should not inherit our exit semantics.

	let crashHandlerRunning = false;

	function logFatalToStderr(kind, err) {
		// Guard against recursive crashes (e.g., logging itself throws).
		if (crashHandlerRunning) {
			try {
				process.stderr.write(`wheat-mcp: recursive crash in ${kind}\n`);
			} catch {
				/* give up */
			}
			process.exit(2);
		...
		try {
			...
			try {
				process.stderr.write(
					`wheat-mcp FATAL ${kind}: ${err?.message || err}\n`,
				);
			} catch {
	...
	process.on("unhandledRejection", (reason) => {
		...
		// Node 20+ default for unhandled rejections is also to terminate; being
		// explicit is safer than relying on the --unhandled-rejections default.
		process.exit(1);
	});

	// Test-only hook: when WHEAT_MCP_CRASH_TEST is set, schedule the
	// corresponding crash shortly after startup so the test suite can verify
	// the handlers above fire correctly. Never triggered in production.
	if (process.env.WHEAT_MCP_CRASH_TEST) {
		const mode = process.env.WHEAT_MCP_CRASH_TEST;
		setTimeout(() => {
			if (mode === "uncaught") {
				throw new Error("WHEAT_MCP_CRASH_TEST uncaught");
			}
			if (mode === "unhandled") {
				Promise.reject(new Error("WHEAT_MCP_CRASH_TEST unhandled"));
			}
		}, 50);
	}
...
export { handleRequest, RESOURCES, startServer, TOOLS };
