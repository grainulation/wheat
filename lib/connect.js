...
`);
  ...
  if (token) {
    console.log(`\n  \x1b[32m\u2713\x1b[0m Auth token ${flags.token ? "provided via --token" : "auto-discovered from .farmer-token"}`);
  ...
  try {
    ...
    console.log(
      `  \x1b[32m+\x1b[0m .farmer-config.json (sprint paths + ${token ? "auth token" : "no token"} registered)`
    );
  } catch (err) {
    console.log(
