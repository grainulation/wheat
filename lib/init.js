...
if git diff --cached --name-only | grep -q 'claims.json'; then
  npx --yes @grainulator/wheat compile --check --dir "${escapedDir}" 2>/dev/null
  if [ $? -ne 0 ]; then
...
async function run(dir, args) {
  ...
  if (flags.headless) {
    // ── Headless mode ──
    if (!flags.question) {
      console.error('  --headless requires --question "..."');
      process.exit(1);
    }
    meta = {
      question: flags.question,
      audience: flags.audience ? flags.audience.split(',').map(s => s.trim()) : ['self'],
      constraints: flags.constraints ? flags.constraints.split(';').map(s => s.trim()) : [],
