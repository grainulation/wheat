...
import { atomicWrite } from "@grainulation/barn/atomic";
...
export function loadClaims(dir, opts = {}) {
	...
	if (result.errors.length === 0 && result.data) {
		...
		if (migrated !== raw) {
			try {
				// Atomic — tmp + rename — so concurrent readers never see
				// a torn file mid-migration. See @grainulation/barn/atomic.
				atomicWrite(claimsPath, migrated + "\n");
			} catch (err) {
